...
# reversing the direction should not affect result => do not use for cache
@cached(cache={}, key=lambda pt, perimeter, reverse_direction=False: hashkey(pt, perimeter))
def point_in_shape(pt: Point, perimeter: frozendict[Point, Point], reverse_direction=None) -> bool:
    if pt in perimeter:
        return True

    if reverse_direction is None:  # optimise based on closer end
        reverse_direction = abs(maxy - pt.y) < abs(miny - pt.y)  # faster in reverse if True

    limit = miny if not reverse_direction else maxy
    range_to_use = range(pt.y, limit - 1, -1) if not reverse_direction else range(pt.y, limit + 1)
    closest_in_dir = None

    for idx in range_to_use:
        testpt = (pt.x, idx)
        if testpt in perimeter:
    ...
    if isinstance(closest_in_dir, VertexPoint):
        ...
        # we know nv1.y = closest_above.y < pt.y (unless reverse_direction then > pt.y)
...
for p1, p2 in combinations(redtiles, 2):
    ...
    if area > part2:
        if p1.x == p2.x or p1.y == p2.y:  # 1-D
            continue  # I highly doubt largest will be 1-D. does mean this is not necessarily generalised!

        dir_x = 1 if p2.x > p1.x else -1
        dir_y = 1 if p2.y > p1.y else -1

        if (dir_x, dir_y) == p1.direction():  # must be convex for rest of rectangle to be inside
            if not p1.convex:
                continue
        else:  # must be concave for rest of rectangle to be inside
            if p1.convex:
                continue
        if (-dir_x, -dir_y) == p2.direction():  # must be convex for rest of rectangle to be inside
            if not p2.convex:
                continue
        else:  # must be concave for rest of rectangle to be inside
            if p2.convex:
        ...
        # if all edges are fully contained in the big shape, then so is the entire rectangle
        # actually because of our 1D edges there could be a "contained" edge that is partly outside but this is unlikely

        # first step: check if other two corners are inside the shape
        p3 = Point(p1.x, p2.y)
        p4 = Point(p2.x, p1.y)

        # we want to look 'towards' p1 for p3, 'towards' p2 for p4 (points with same x) to save time
        # default in point_in_shape is checking towards decreasing y. so we need to reverse if dir_y is -1 (going down from 1 to 3, so check should go from 3 to 1 => increasing y)
        # if both true, faster to go towards p1 which is other way
        rev_dir_p3 = abs(miny - p3.y) > abs(p3.y - p1.y) and dir_y == -1
        rev_dir_p4 = abs(miny - p4.y) > abs(p4.y - p2.y) and dir_y == 1

        if not point_in_shape(p3, outerloop, rev_dir_p3) or not point_in_shape(p4, outerloop, rev_dir_p4):
            continue
        ...
        for x in range(p1.x + dir_x, p2.x - 2 * dir_x, dir_x):
            ...
            if (x, p1.y) in outerloop and not point_in_shape(Point(x + dir_x, p1.y), outerloop):
                continue_flag = True
                break
            if (x, p2.y) in outerloop and not point_in_shape(Point(x + dir_x, p2.y), outerloop):
                continue_flag = True
                break
        ...
        for y in range(p1.y + dir_y, p2.y - 2 * dir_y, dir_y):
            if (p1.x, y) in outerloop and not point_in_shape(Point(p1.x, y + dir_y), outerloop):
                continue_flag = True
                break
            if (p2.x, y) in outerloop and not point_in_shape(Point(p2.x, y + dir_y), outerloop):
                continue_flag = True
                break
...
# WARNING: this may take around 2 minutes to run!
print("Part 1:", part1)
print("Part 2:", part2)
